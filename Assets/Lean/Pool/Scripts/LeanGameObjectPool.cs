    ...
    public class LeanGameObjectPool_Inspector : LeanInspector<LeanGameObjectPool> {
        protected override void DrawInspector() {
            ...
            if (Draw("prefab", "The prefab this pool controls.") == true) {
                Each(t => { t.Prefab = (GameObject) serializedObject.FindProperty("prefab").objectReferenceValue; },
                    true);
            }

            EndError();
            Draw("Notification",
                "If you need to peform a special action when a prefab is spawned or despawned, then this allows you to control how that action is performed. None = If you use this then you must rely on the OnEnable and OnDisable messages. SendMessage = The prefab clone is sent the OnSpawn and OnDespawn messages. BroadcastMessage = The prefab clone and all its children are sent the OnSpawn and OnDespawn messages. IPoolable = The prefab clone's components implementing IPoolable are called. Broadcast IPoolable = The prefab clone and all its child components implementing IPoolable are called.");
...
namespace Lean.Pool {
    ...
    ///     Pools also have settings to preload, recycle, and set the spawn capacity, giving you lots of control over your
    ///     spawning.
    /// </summary>
    ...
    public class LeanGameObjectPool : MonoBehaviour, ISerializationCallbackReceiver {
        ...
        /// <summary>
        ///     If you need to peform a special action when a prefab is spawned or despawned, then this allows you to control how
        ...
        /// <summary>
        ///     This stores all spawned clones in a list. This is used when Recycle is enabled, because knowing the spawn
        ///     order must be known. This list is also used during serialization.
        /// </summary>
        [SerializeField] private List<GameObject> spawnedClonesList = new List<GameObject>();

        /// <summary>
        ///     This stores all spawned clones in a hash set. This is used when Recycle is disabled, because their storage
        ///     order isn't important. This allows us to quickly find the Clone associated with the specified GameObject.
        /// </summary>
        ...
        /// <summary>This will either spawn a previously despanwed/preloaded clone, recycle one, create a new one, or return null.</summary>
        public bool TrySpawn(Vector3 position, Quaternion rotation, Transform parent, bool worldPositionStays,
        ...
        protected virtual void Update() {
            ...
            for (var i = delays.Count - 1; i >= 0; i--) {
                ...
                if (delay.Clone != null) {
                    ...
                    if (Warnings == true) {
                        Debug.LogWarning(
                            "Attempting to update the delayed destruction of a prefab clone that no longer exists, did you accidentally delete it?",
                            this);
                    }
                }
            }
